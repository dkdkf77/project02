...
SDE.Ghost = function() {
    var oMessages = (typeof parent.SDE === 'object' && typeof parent.SDE.Message === 'object' && typeof parent.SDE.Message.Ghost === 'object')
        ? parent.SDE.Message.Ghost
        : {};

    var LANGUAGE_CODE = (typeof CAFE24.GLOBAL_INFO === 'object' && typeof CAFE24.GLOBAL_INFO.getAdminLanguageCode === 'function')
        ? CAFE24.GLOBAL_INFO.getAdminLanguageCode()
        : 'ko_KR';
    var BTN_WIDTH = 170,
        BTN_HEIGHT = 67,
        ...
        DUMMY_TEMPLATE = '<div style=\'padding:65px 0 75px; border:1px solid #ffdeb7; background:url("//img.echosting.cafe24.com/design/common/bg_add_module_line.gif") repeat 0 0; text-align:center;\'>' +
            ...
            '<img src="//img.echosting.cafe24.com/design/common/'+ LANGUAGE_CODE +'/txt_add_module.gif" alt="' + oMessages.clickEditTheScreen + '">' +
            '</p>' +
            '</div>',

        TEMPLATE = '<div class="field" style="display:none;">' +
            '<div class="editorBtn" style="padding: 0 35px 0 0;">' +
            '<p class="btn"><button type="button" class="edit">' + oMessages.edit + '</button></p>' +
            ...
            '</div>' +
            '<p class="closeBtn"><button type="button" class="close" title="' + oMessages.close + '">' + oMessages.close + '</button></p>' +
            '</div>';
    ...
    var action = function(actionType) {
        ...
        if (!utilModule.has(type, key)) {
            ...
            if (!result || !result.file) {
                alert(oMessages.wasFoundMakeSure);
            } else {
    ...
    var actionClose = function(key) {
        if (!confirm(oMessages.areSureModules)) return;
    ...
    var getModuleType = function() {
        if ($module && $module.prop('tagName') === 'IMG') {
            return 'image';
        }
    ...
    var getModuleIndex = function(key, type) {
        ...
        if (type === 'image' && $target.length > 0 && EC$($target[0]).closest('.xans-editor-dndswiper').length > 0) {
            return 0;
        }
        for (index in $target) {
    ...
    var init = function() {
        ...
        try {
            if (
                typeof parent.SDE.Util.Module === 'undefined' ||
        ...
        EC$(document.body).css({"overflow-y": "scroll"});
    };
     ...
     */
    var render = function() {
        $ghost = EC$(TEMPLATE).appendTo(document.body);

        $size = $ghost.find('.info span');

        $ghost.find('button').click(EC$.proxy(onClickButton, this));

        $ghost
            .click(EC$.proxy(onClickGhost, this))
            .mouseleave(EC$.proxy(onLeaveGhost, this));

        if (SDE.mo()) {
            $ghost
                .find('.editorBtn').css({padding: '0 35px 0 0'}).end()
                .find('.arrowBtn').css({position: 'absolute', right: 20, top: 8})
                .find('a').css({display: "block", overflow: "hidden", width: 28, height: 18, background: 'url("//img.echosting.cafe24.com/smartAdmin/img/editor/' + LANGUAGE_CODE + '/sfix_btn.png") no-repeat', "text-indent": '150%', cursor: 'pointer'})
                .click(EC$.proxy(onClickArrange, this))
    ...
    var restrict = function() {
        // 링크 클릭 제한
        EC$('a, button').click(function(evt) {
            evt.preventDefault();
            evt.stopPropagation();
        });

        // Floating Banner 움직이지 않게 처리
        setTimeout(function() { EC$(window).off('scroll'); }, 300);
    };
    ...
    var show = function($target) {
        ...
        if (offset.left === 0 && offset.top === 0 && EC$(window).height() < $module.height()) {
            $ghost
                .attr('class', 'field')
                .css({
                    top: offset.top + 4,
        ...
        if (SDE.mo() && $target.closest("#contents")[0] && $target.prop('tagName') !== 'IMG') {
    ...
    var inner = function($target) {
        ...
        $target.find('.xans-element-').filter(":not(.xans-record-)").each(function($i, $item) {
            ...
            EC$('<div><div class="-panel"></div><div class="-cmd" style="cursor:pointer !important;_cursor:hand;position:absolute;right:-2px;top:-2px;width:8px;height:8px;border:2px solid #f44;"></div></div>').addClass("innermodule")
                .css({
                    position: "absolute",
    ...
    var setEventHandler = function() {
        var oList = EC$('.xans-element-, img').filter(function () {
        ...
        oList.mouseover(EC$.proxy(onOverModule, this));
    };

    var onClickButton = function(evt) {
        var actionType = EC$(evt.currentTarget).prop('className');
    ...
    var onOverModule = function(evt) {
        evt.stopPropagation();
        var $target = EC$(evt.currentTarget);
    ...
    var onLeaveGhost = function(evt) {
        if (EC$.contains($ghost[0], evt.toElement)) return;
    ...
    var onClickArrange = function(evt) {
        evt.stopPropagation();
        var $dir = {up: -1,down: 1}[EC$(evt.target).data("cmd")];
    ...
    var splitter = function ($target) {
        ...
        var key, type = "module";
        try {
            key = ($target.prop('tagName') === 'IMG' && (type = 'image'))?
                $target.attr('src'):
    ...
    var plugins = {
        "/layout/basic/sidebar.html": function () {
            EC$('#dimmedSlider').show();
            EC$('html').addClass('expand');
            EC$('#aside').css({'visibility': 'visible'});
        }
    };
